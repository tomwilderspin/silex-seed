...
RUN ["yum", "-y", "install", "nginx", "git", "curl", "curl-devel", "wget", "php", "php-common", "php-curl", "php-mysql", "php-devel", "php-gd", "php-pecl-memcache", "php-pspell", "php-snmp", "php-xmlrpc", "php-xml","hhvm"]
...
RUN cd /var/www && hhvm /var/www/composer install --no-ansi --no-dev --no-interaction --no-progress --no-scripts --optimize-autoloader
